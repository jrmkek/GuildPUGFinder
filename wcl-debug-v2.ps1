...
param(
    ...
    [string]$Metric = "",      # e.g. "hps", "dps" - blank = omit
    [string]$Role = "",        # e.g. "Healer", "Tank", "DPS" - blank = omit
    [switch]$ListZones,
    [switch]$Introspect,       # dump valid enum values for metric/role
...
"@
    ...
    Write-Host "`nValid 'metric' values:" -ForegroundColor Cyan
    $result.data.metricType.enumValues.name | ForEach-Object { Write-Host "  $_" }
...
if ($Metric -ne "") {
    $zrArgs += "metric: `$metric"
    $varDecls += "`$metric: CharacterPageRankingMetricType!"
    $variables["metric"] = $Metric
}
if ($Role -ne "") {
    $zrArgs += "role: `$role"
    $varDecls += "`$role: RoleType!"
    $variables["role"] = $Role
}
